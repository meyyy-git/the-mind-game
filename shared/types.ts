...
export type Action = { id: string; epoch: number; gameId: string; type: 'ready' | 'start' | 'play' | 'propose' | 'vote' | 'resume' | 'lobby' | 'cancel' | 'kick' | 'leave' | 'seat' | 'settings'; card?: number; yes?: boolean; target?: string; settings?: Settings };
